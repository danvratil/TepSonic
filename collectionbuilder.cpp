...
void CollectionBuilder::run()
{
    ...
    QSqlQuery artistsQuery("SELECT `artist` FROM `Tracks` GROUP BY `artist` ORDER BY `artist` DESC",dbManager.sqlDb);
    while (artistsQuery.next()) {
        albumsParent = _model->addRow(QModelIndex(),artistsQuery.value(0).toString(),QString());
        QSqlQuery albumsQuery("SELECT `album` FROM `Tracks` WHERE `artist`='"+artistsQuery.value(0).toString()+"' GROUP BY `album` ORDER BY `album` DESC;",dbManager.sqlDb);
        while (albumsQuery.next()) {
            tracksParent = _model->addChild(albumsParent,albumsQuery.value(0).toString(),QString());
            QSqlQuery tracksQuery("SELECT `title`,`filename` FROM `Tracks` WHERE `album`='"+albumsQuery.value(0).toString()+"' AND `artist`='"+artistsQuery.value(0).toString()+"' ORDER BY `trackNo` DESC;",dbManager.sqlDb);
